...
let BilldepositComponent = class BilldepositComponent {
    constructor(common, fb, ngxService) {
        this.common = common;
        this.fb = fb;
    ...
    GetProviderlist() {
        this.common.PostMethod("GetfilterData", { file: "service_provider", name: "provider_type", value: "Electricity Payment", orderby: "asc" }).then((res) => {
            this.lists.Providerlist = res.Data;
        });
    }
    ngAfterViewInit() {
    ...
    GetBillinfo() {
        ...
        if (this.Billform.value.provider_Id == 'Electricity') {
            this.common.PostMethod("SearchProviderInfo", { number: this.Billform.value.kno }).then((res) => {
                this.ngxService.stopAll();
                let data = JSON.parse(res.Data);
                if (data.customername) {
                    data.name = data.customername;
                    data.amount = data.dueamount;
                    data.number = this.Billform.value.kno;
            ...
            this.common.PostMethod("GetWaterBill", { number: this.Billform.value.kno }).then((res) => {
                this.ngxService.stopAll();
                let data = JSON.parse(res.Data);
                console.log(data);
                this.lists.BillCustInfo.name = data.records[0].CustomerName;
                this.lists.BillCustInfo.amount = data.records[0].Netamount;
                this.lists.BillCustInfo.number = this.Billform.value.kno;
                this.lists.BillCustInfo.duedate = data.records[0].Duedate;
                this.lists.BillCustInfo.billdate = data.records[0].Billdate;
                this.Billform.controls['kno'].disable();
    ...
    BillSubmit() {
        ...
        this.common.PostMethod("BillDeposit", this.Billform.value).then((res) => {
            if (res.Status == 1) {
                this.print(this.Billform.value);
...
BilldepositComponent.ctorParameters = () => [
...
//# sourceMappingURL=Portal-Bill-bill-module-es2015.js.map